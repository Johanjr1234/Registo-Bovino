...
function actualizarFinanzas(data) {
    ...
    data.forEach(a => {
        // Sumamos todas las compras (incluso si el animal ya se vendió, fue un gasto inicial)
        if (a.precioCompra) totalCompras += parseFloat(a.precioCompra);
        
        // Sumamos las ventas solo de los vendidos
        if (a.estado === "VENDIDO" && a.precioVenta) totalVentas += parseFloat(a.precioVenta);
    });

    // Actualizar Textos
    const elCompras = document.getElementById('total-compras');
    const elVentas = document.getElementById('total-ventas');
    if (elCompras) elCompras.innerText = formatCOP(totalCompras);
    if (elVentas) elVentas.innerText = formatCOP(totalVentas);

    // Actualizar Gráfico
    const ctx = document.getElementById('balanceChart');
    if (ctx) {
        if (myChart) myChart.destroy(); // Destruir anterior para no sobreponer
        
        // Crear gráfico nuevo
        myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Inversión (Compras)', 'Ganancia (Ventas)'],
                datasets: [{
                    label: 'Dinero en COP',
...
if (form) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            form.reset(); cargarOpcionesPadres();
...
window.cargarInventario = async (verHistorial = false) => {
    ...
    try {
        ...
        // LLAMADA A LA FUNCIÓN FINANCIERA
                        ...
                        <div class="mini-cria-card">
                            <div class="mini-header" onclick="event.stopPropagation(); window.toggleChildActions('${h.id}')">
                                ...
                                <div class="mini-info">
                                    ...
                                    <small style="color: #666; margin-left: 5px;">(${hFecha})</small>
                                </div>
                            </div>
